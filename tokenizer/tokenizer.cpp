			...
			case INITIAL_STATE: {
				...
				//先排除0
					...
					case '<':{
                        ...
                        if(ch == '='){
                            return std::make_pair(std::make_optional<Token>(TokenType::SMALL_EQUAL_SIGN,'<', pos, currentPos()), std::optional<CompilationError>());
                        }
                        unreadLast();
                        return std::make_pair(std::make_optional<Token>(TokenType::SMALL_SIGN,'<', pos, currentPos()), std::optional<CompilationError>());
                    }
					  case '>':{
                        current_char = nextChar();
                        ch = current_char.value();
                        if(ch == '='){
                            return std::make_pair(std::make_optional<Token>(TokenType::MORE_EQUAL_SIGN,'>', pos, currentPos()), std::optional<CompilationError>());
                        }
                        unreadLast();
...
/*                    std::string s ;
			...
			case UNSIGNED_INTEGER_STATE: {
				...
				if(!current_char.has_value()){
				    ...
				    int a = 0 ;
				    while(str[a]=='0') a++ ;
				    if(str.length() - a > 10 || (str.length() == 10 && str > "2147483647"))
                        return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrIntegerOverflow));
                    int b = atoi(str.c_str());
                    return std::make_pair(std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, b, pos, currentPos()), std::optional<CompilationError>());
                }
				auto ch = current_char.value();
			...
			case IDENTIFIER_STATE: {
				...
				if(!current_char.has_value()){
                    ...
                    if(str == "const")
                        ...
                        return std::make_pair(std::make_optional<Token>(TokenType::BREAK, "break", pos, currentPos()), std::optional<CompilationError>());
                    else if(str == "continue")
                        return std::make_pair(std::make_optional<Token>(TokenType::CONTINUE, "continue", pos, currentPos()), std::optional<CompilationError>());
                    else if(str == "print")
                        return std::make_pair(std::make_optional<Token>(TokenType::PRINT, "print", pos, currentPos()), std::optional<CompilationError>());
                    else if(str == "scan")
                        return std::make_pair(std::make_optional<Token>(TokenType::SCAN, "scan", pos, currentPos()), std::optional<CompilationError>());
                    else
            ...
            case DIVISION_SIGN_STATE:{
                ...
                ///**/处理
                    ...
                    ss >> s;
                    ss.clear();
                    bool flag = false; //如果是注释则退出
                    do{
                        do{
                            current_char = nextChar();
                            if(!current_char.has_value()){
                                return std::make_pair(std::optional<Token>(), std::make_optional<CompilationError>(pos, ErrorCode::ErrZhuShi));
                            }
                            ch = current_char.value();
                        }while(ch != '*');
                        do{
            ...
            case EQUAL_SIGN_STATE:{
                auto ch = current_char.value();
                if(ch == '=')
                    return std::make_pair(std::make_optional<Token>(TokenType::EQUAL_EQUAL_Sign, '=', pos, currentPos()), std::optional<CompilationError>());
                unreadLast();
	...
	int Tokenizer::judgeHexNum(std::string s){
